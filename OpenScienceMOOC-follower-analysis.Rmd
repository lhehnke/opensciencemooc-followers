...
knitr::opts_chunk$set(echo = TRUE, error = TRUE, warning = FALSE, message = FALSE, fig.width = 9, fig.height = 6, (root.dir = paste0(rprojroot::find_rstudio_root_file(), "/Files")))
```
...
```{r authentication, eval = FALSE, include = FALSE}
...
followers_df <- twListToDF(osci_followers)
...
# Export data
saveRDS(followers_df, "opensciencemooc_followers.rds")
saveRDS(friends_df, "opensciencemooc_friends.rds")
```

```{r}
# Import Twitter data
data_df <- readRDS(file = paste(MAIN_DIR,"opensciencemooc_followers.rds", sep = "/"))
```
...
```{r friends-followers, fig.align = "center", fig.width = 10}
# Import data on friends and merge
friends_df <- readRDS(file = paste(MAIN_DIR,"opensciencemooc_friends.rds", sep = "/"))
...
```{r locations, fig.align = "center", fig.width = 10}
...
```{r, eval = FALSE}
# Geocode cleaned locations and save results
locations_geo <- geocode_OSM(data_df$location_clean, as.data.frame = TRUE, details = TRUE)
saveRDS(locations_geo, "locations_geocoded.rds")
```

```{r}
# Import geocoded data
locations_geo <- readRDS(file = paste(MAIN_DIR,"locations_geocoded.rds", sep = "/"))
...
```{r locations-static-map1, fig.align = "center", fig.width = 10}
# Get world map
...
```{r locations-interactive-map, fig.align = "center", fig.width = 10}
...
# Export data
saveRDS(followers_gender, "followers_gender.rds")
```
...
```{r gender, fig.align = "center", fig.width = 10}
# Import genderized data
followers_gender <- readRDS(file = paste(MAIN_DIR,"followers_gender.rds", sep = "/"))
...
```{r account-status, fig.align = "center", fig.width = 10}
...
```{r influential-followers, fig.align = "center", fig.width = 10}
...
```{r active-followers, fig.align = "center", fig.width = 10}
...
```{r correlation, fig.align = "center", fig.width = 10}
data_df %>%
...
```{r word-counts, fig.align = "center", fig.width = 10}
...
```{r word-cloud, fig.align = "center", fig.width = 10}
# Plot word cloud
desc_tidy %>%
  count(word_stem, sort = TRUE) %>%
  top_n(100, n) %>%
...
```{r mentions, fig.align = "center", fig.width = 10}
...
```{r bigram-counts, fig.align = "center", fig.width = 10}
...
```{r bigram-network, fig.align = "center", fig.width = 10}
...
```{r sentiment-counts, fig.align = "center", fig.width = 10}
...
```{r sentiment-distribution, fig.align = "center", fig.width = 10}
...
```{r comparison-cloud, fig.align = "center", fig.width = 10}
